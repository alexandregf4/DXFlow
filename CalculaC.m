function [C] = CalculaC(opcaoDesacoplado, l, c, barrasDeltaPQ, dadosEntrada)

%% Cabeçalho

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% Esta função calcula C para a linha (l) e coluna (c) correspondente da
%%%% Jacopbiana.
...
if ~isempty(opcaoDesacoplado)
    ...
    if l <= dadosEntrada.npq+dadosEntrada.npqv+dadosEntrada.npv                                                                            % Erro se o índice do vetor de estados não estiver entre as variáveis convencionais
        ...
        indice_linha_chaveavel = l - (dadosEntrada.npq + dadosEntrada.npqv+ dadosEntrada.npv);
    end
...
%%%% Se a função for utilizada no método convencional
    ...
    if l <= 2*dadosEntrada.npq+2*dadosEntrada.npqv+dadosEntrada.npv                                                                        % Erro se o índice do mismatch nao estiver entre os mismatches chaveáveis
        ...
        indice_linha_chaveavel = (indice_mismatches + 1)./2;                                        % Transformação do índice do vetor de mismatches para o índice das linhas chaveáveis
    end
end
...
if dadosEntrada.statusChaves(indice_linha_chaveavel) == 1                                % Se chave aberta (C apenas se aplica para chaves abertas)
