function [O] = CalculaO(opcaoDesacoplado, l, c, dadosEntrada)
...
if ~isempty(opcaoDesacoplado)
    ...
    if l <= dadosEntrada.npq+dadosEntrada.npqv+dadosEntrada.npv                                                                            % Erro se o índice do vetor de estados não estiver entre as variáveis convencionais
        error('Erro! Índice c não pertence ao intervalo adequado do vetor de estados!');
    else
        indice_linha_chaveavel_mismatches = l - (dadosEntrada.npq + dadosEntrada.npqv+ dadosEntrada.npv);
    end
...
%%%% Se a função for utilizada no método convencional
    ...
    indice_linha_chaveavel_mismatches = (indice_mismatches+1)./2;                                   % Transformação do índice do vetor de mismatches para o índice das linhas chaveáveis
end
...
if ~isempty(opcaoDesacoplado)
    ...
    if c <= dadosEntrada.npq+dadosEntrada.npqv+dadosEntrada.npv                                                                            % Erro se o índice do vetor de estados não estiver entre as variáveis convencionais
        ...
        indice_linha_chaveavel_xvar = c - (dadosEntrada.npq + dadosEntrada.npqv + dadosEntrada.npv);
    end
    ...
    if c <= 2*dadosEntrada.npq+2*dadosEntrada.npqv+dadosEntrada.npv                                                                        % Erro se o índice do vetor de estados não estiver as variáveis de estado dos ramos chaveáveis
        ...
        indice_linha_chaveavel_xvar = (indice_xvar+1)./2;                                           % Transformação do índice do vetor xvar para o índice das linhas chaveáveis
    end
end
...
if dadosEntrada.statusChaves(indice_linha_chaveavel_mismatches) == 0                                         % Se chave fechada (O apenas se aplica para chaves fechadas)
