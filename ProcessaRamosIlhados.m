...
if metodo == 1
    ...
    numeroIlhas = 0;
    for k=1:length(U)
        if abs(U(k,k)) <= 1e-5 % pivos nulos
            numeroIlhas = numeroIlhas + 1;
            U(k,k) = 1;
            b(k,1) = numeroIlhas;
        end
    end

    tempBarrasIlha = round(inv(U)*b,0);
    ...
    for k=1:numeroIlhas
       contadorBarras = 1;
       for l=1:length(tempBarrasIlha)
          if tempBarrasIlha(l,1) == k
            barrasIlha{k,1}(contadorBarras,1) = l;
            contadorBarras = contadorBarras + 1;
          end
       end
       contadorLinhas = 1;
       for l=1:length(barrasIlha{k,1})
           tempLinhas = find(dadosEntrada.de == barrasIlha{k,1}(l,1));
           for m=1:length(tempLinhas)
               if ~isempty(linhasIlha{k,1})
                   if ~isempty(find(tempLinhas(m,1) == linhasIlha{k,1}))
                       continue
                   end
               end
               if ~isempty(find(dadosEntrada.linhasChaveaveis == tempLinhas(m,1)))
                   if find(barrasIlha{k,1} == dadosEntrada.para(tempLinhas(m,1)))
                       linhasIlha{k,1}(contadorLinhas,1) = tempLinhas(m,1);
           ...
           tempLinhas = find(dadosEntrada.para == barrasIlha{k,1}(l,1));
           for m=1:length(tempLinhas)
               if ~isempty(linhasIlha{k,1})
                   if ~isempty(find(tempLinhas(m,1) == linhasIlha{k,1}))
                       continue
               ...
               if ~isempty(find(dadosEntrada.linhasChaveaveis == tempLinhas(m,1)))
                   if find(barrasIlha{k,1} == dadosEntrada.de(tempLinhas(m,1)))
                       linhasIlha{k,1}(contadorLinhas,1) = tempLinhas(m,1);
                       contadorLinhas = contadorLinhas + 1;
                   end
               else
       ...
       linhasIlha{k,1} = sort(linhasIlha{k,1});
    end
    
    tempoIdentificacaoIlhas = toc;
    fprintf('\nMétodo de identificação de ilhas: Monticelli');
    fprintf('\nTempo de processamento de ilhas: %f segundos',tempoIdentificacaoIlhas);
else
    tic
    ...
    tempoIdentificacaoIlhas = toc;
    fprintf('\nMétodo de identificação de ilhas: Busca em profundidade');
    fprintf('\nTempo de processamento de ilhas: %f segundos',tempoIdentificacaoIlhas);
end
...
if numeroIlhas > 1
    ...
    for k=1:numeroIlhas
        for l=1:length(barrasIlha{k,1})
            ...
            elseif dadosEntrada.tipoBarra(dadosEntrada.barras(barrasIlha{k,1}(l,1))) == 2
                ...
                if isempty(find(ilhasComReferencia == k))
                    ilhasComReferencia(contadorIlhasComReferencia, 1) = k;
                    contadorIlhasComReferencia = contadorIlhasComReferencia + 1;
                end
            end
    ...
    for k=1:length(ilhasComReferencia)
        if ilhaReferenciaVT == ilhasComReferencia(k)
            referenciasIlhas(k,1) = find(dadosEntrada.tipoBarra == 1);
        else
            referenciasIlhas(k,1) = max(listaBarrasPVIlhas{ilhasComReferencia(k),1});
        end
    end
